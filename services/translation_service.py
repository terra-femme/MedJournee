...
def match_language_role(
    ...
    Each side has an explicit "speaks" language (for detection matching) and
    a "translate_to" language (the subtext translation target).

    Returns {"speaker_role": "provider"|"family"|"unknown", "translate_to": <lang_code>}
    """
    if not detected_lang:
        # Fallback: assume provider, translate to provider's translate_to target
        return {"speaker_role": "provider", "translate_to": provider_translate_to}

    # Normalize Chinese variants: Whisper returns "zh", user may configure "zh-cn" or "zh-tw"
    def normalize(code):
    ...
    if det_norm == prov_norm:
        # Provider is speaking — translate into provider's translate_to (for family to read)
        return {"speaker_role": "provider", "translate_to": provider_translate_to}
    elif det_norm == fam_norm:
        # Family is speaking — translate into family's translate_to (for provider to read)
        return {"speaker_role": "family", "translate_to": family_translate_to}
    else:
