...
class RealtimeTranscriptionService:
        ...
        try:
            ...
            try:
                result = subprocess.run([
                    'ffmpeg',
                    '-loglevel', 'error',
                    '-f', input_ext,           # Force input format (avoids header mismatch issues)
                    '-i', temp_input.name,
                    '-ar', '16000',
                    '-ac', '1',
                    '-c:a', 'pcm_s16le',
                ...
                if result.returncode == 0 and os.path.exists(temp_output.name) and os.path.getsize(temp_output.name) > 1000:
                    ...
                    print(f"[Instant] FFmpeg failed (rc={result.returncode}): {stderr_msg}")
            ...
            # If FFmpeg failed and original is webm, skip sending to OpenAI
            # (raw webm chunks without proper headers get rejected)
            if not ffmpeg_success and input_ext == "webm":
                print(f"[Instant] Skipping OpenAI - FFmpeg failed and raw webm likely invalid")
                return {
                    "success": True,
                    "text": "",
                    "is_empty": True,
                    "reason": "Audio conversion failed, waiting for more data"
                }
            ...
            # Prepare API call parameters - verbose_json returns detected language
            api_params = {
                "model": "whisper-1",
                "file": file_tuple,
            ...
            # No language hint — let Whisper auto-detect for bidirectional support
    ...
    async def transcribe_and_translate_instant(
        ...
        """
        Fast transcription + bidirectional translation for real-time display.
        Detects which language is spoken, translates to the configured target.
        """
        from services.translation_service import translate_text, match_language_role

        # Step 1: Fast transcription (auto-detect language)
        transcription_result = await self.transcribe_chunk_instant(audio_file)
        ...
        # Step 2: Determine speaker role and translation direction
        role_info = match_language_role(
            detected_language, provider_spoken, provider_translate_to,
            family_spoken, family_translate_to
        )
        speaker_role = role_info["speaker_role"]
        translate_to = role_info["translate_to"]

        # Step 3: Translate to the configured target language
        try:
            translation_result = await translate_text(original_text, translate_to, None)
    ...
    async def _enhance_with_enrollment(
        ...
        for segment in diarized_segments:
            ...
            try:
                ...
                # Lowered threshold from 0.70 to 0.65 for better voice matching
